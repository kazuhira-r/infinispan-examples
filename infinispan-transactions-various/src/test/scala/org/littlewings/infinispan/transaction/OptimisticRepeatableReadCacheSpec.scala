...
class OptimisticRepeatableReadCacheSpec extends FunSpec
                                        with InfinispanCacheSupport[String, String]
                                        with ThreadSupport {
  describe("optimistic repeatable-read cache spec") {
    it("test") {
      withCache("optimisticRepeatableReadCache") { cache =>
        ...
        result1.get.get should be (5)
        result2.get.get should be (5)

        cache.get("key1") should be ("value1")
        cache.get("key2") should be ("value2")
        cache.get("key3") should be ("value3")
        cache.get("key5") should be ("value5-2")
